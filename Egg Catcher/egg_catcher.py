...
canvas_width = 800
canvas_height = 400
root = Tk() #this creates a window
c = Canvas(root, width = canvas_width, height= canvas_height,\
    background='deep sky blue')
c.create_rectangle(-5, canvas_height - 100, canvas_width + 5,\
    canvas_width + 5, fill='sea green', width=0)
c.create_oval(-80, -80, 120, 120, fill='orange', width=0)
c.pack()

#setting up eggs
color_circle = cycle(['light blue','light green', 'light pink', 'light yellow', 'light cyan'])
egg_width = 45
egg_height = 55
egg_score = 10
egg_speed = 500
...
#Falling, scoring and dropping
eggs = []

#Function: creating eggs
def create_egg():
    x = randrange(10,740)
    y = 40
    new_egg = c.create_oval(x, y, x + egg_width, y + egg_height, fill=next(color_circle), width=0)
    eggs.append(new_egg)
    root.after(egg_interval, create_egg)
    
#Function: Moving eggs
def move_eggs():
    for egg in eggs:
...
#Function: Lose a life
def lose_a_life():
    global lives_remaining
    lives_remaining -= 1
    c.itemconfigure(lives_text, text='Lives: ' + str(lives_remaining))
    
#Function: check for a catch
def check_catch():
    global eggs
    ...
    for egg in eggs:
        if egg in eggs:
            (egg_x, egg_y, egg_x2, egg_y2) = c.coords(egg)
            if catcher_x < egg_x2 and egg_x2 < catcher_x2 and catcher_y2 - egg_y2 < 40:
